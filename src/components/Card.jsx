    ...
    return (
        <section className='grid-cards'>
            {namesArray && namesArray.map((value, index) => {
                return (
                    <>
                        <span id={value} className='card-class' key={index}
                            onMouseOut={() => removeAll(value)} onClick={() => addContentEditable(value)}>{value}
                            <svg style={{ marginTop: 'auto' }} xmlns="http://www.w3.org/2000/svg" onClick={() => maxSize(value)} viewBox="0 0 20 20" fill="currentColor" height={12} width={12}>
